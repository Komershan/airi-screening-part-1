    ...
    with h5py.File(fname, "w") as f:
        for mapping_key, mapping_value in mapping.items():
            ...
            if mapping_value == 'array':
                ...
                f.create_dataset(mapping_key, data=object_params[mapping_key], compression=compression)
            elif mapping_value == 'bool':
                f.create_dataset(mapping_key, data=np.array([object_params[mapping_key]], dtype=np.bool_), compression=compression)
            else:
                assert hasattr(np, f"{mapping_value}64")
                f.create_dataset(mapping_key, data=np.array([object_params[mapping_key]], dtype=getattr(np, f"{mapping_value}64")), compression=compression)
...
class HistoriesDataset:
    ...
    def write(self, fname, compression="gzip"):
        object_params = {key:value for key, value in self.__dict__.items() if not key.startswith('__') and not callable(key)}
        write_h5(object_params, fname, mapping=self.mapping_dict, compression=compression)
    ...
    def __getitem__(self, index):
        ...
        if self.resize_history is None:
            ...
            return torch.tensor(self.histories[index * 3:(index + self.resize_history) * 3].tolist(), dtype=torch.long)
